...
def clean_and_analyze():
    ...
    try:
        ...
        db_stats_path = os.path.join(config.DATA_DIR, "db_stats.json")
        with open(db_stats_path, "w", encoding="utf-8") as f:
            _json.dump(db_stats, f, ensure_ascii=False, indent=2)
        logger.info(f"数据库统计已导出: {db_stats_path}")
        ...
        if sent_stats["total"] > 0:
            ...
            stats_path = os.path.join(config.DATA_DIR, "sentiment_stats.json")
            with open(stats_path, "w", encoding="utf-8") as f:
                json.dump(sent_stats, f, ensure_ascii=False, indent=2)
            logger.info(f"情感统计已导出: {stats_path}")

            keywords = sentiment_report.get("top_keywords", {})
            if keywords:
                kw_path = os.path.join(config.DATA_DIR, "keywords.csv")
                with open(kw_path, "w", encoding="utf-8-sig", newline="") as f:
                    w = _csv.writer(f)
                    w.writerow(["关键词", "词频"])
                    for k, v in keywords.items():
                        w.writerow([k, v])
                logger.info(f"关键词已导出: {kw_path}")
